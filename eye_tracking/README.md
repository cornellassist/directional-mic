...
## How to run:
### Conda environment
1. `conda create -n py32 python=3.7 --platform win-32`
2. `conda activate py32`
3. `pip install -r requirements.txt`
4. `python -c "import sys; print(sys.executable)"`. This should return a path.
5. Replace the value in `PYTHON_32BIT` in `GazeTracker.py` with the path from step 4. Example: `PYTHON_32BIT = r"C:\Users\brian\anaconda3\envs\py32\python.exe"`
### Tobii Eye tracker 5
1. Download the driver from https://gaming.tobii.com/getstarted/?srsltid=AfmBOoqtHWQ9VsH0qeV-HSXZ5jaxlxJ1h7wBScOTux0N3qusQhJdJTXi
    1. Select the hardware: Tobii Eyetracking
    2. Select Tobii device: Tobii Eye Tracker 5 
    3. Download Tobii Experience Driver v1.133.
2. Download Tobii Experience app from Microsoft store (Not Tobii Gamehub or Tobii Ghost): https://apps.microsoft.com/detail/9nk75kf67s2n. Make sure to install the driver before installing the app.
### Run the arduino
1. Run the `arduino/wheelchair_control_serial/wheelchair_control_serial.ino` or `arduino/wheelchair_control_AP_wifi/wheelchair_control_AP_wifi.ino`
### Python interface
1. `pip install -r python/python_interface/requirements.txt`
2. Then run the python script: `python/python_interface/python_interface_gaze.py`
---
- When the Python program starts, the arduino will output "Neutral". Turn on the wheelchair at this stage. 
- When the wheelchair passes the self-check (all lights are on), press "Enter" and you should be able to control the wheelchair with your eyes!
